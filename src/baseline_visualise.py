"""
baseline_visualise.py — HeraSched result visualiser

Usage
-----
# Auto-detect mode (single CSV in result/ → single-run; multiple → multi-run):
    python baseline_visualise.py

# Explicit single file:
    python baseline_visualise.py --file result/fcfs+best_fit.csv

# Custom result directory:
    python baseline_visualise.py --result-dir /path/to/result

# Multi-run comparison across a custom directory:
    python baseline_visualise.py --result-dir /path/to/result --mode multi

# Skip interactive window (headless / batch):
    python baseline_visualise.py --no-show
...
METRIC_COLS = ["node_utilization", "cpu_utilization", "gpu_utilization", "mem_utilization"]
...
def run_single(csv_path: Path, plots_dir: Path, show: bool) -> None:
    ...
    fig_hist, axes = plt.subplots(nrows, ncols, figsize=(6 * ncols, 4 * nrows), squeeze=False)
    axes_flat = axes.flatten()

    for i, col in enumerate(active):
        ax = axes_flat[i]
        values = df[col].dropna()
        ax.hist(values, bins=60, color=METRIC_COLOURS[col], edgecolor="none", alpha=0.85)
        ax.axvline(values.mean(), color="black", linewidth=1.2, linestyle="--", label=f"mean={values.mean():.3f}")
...
def run_multi(csv_paths: list[Path], plots_dir: Path, show: bool) -> None:
    ...
    col_w = 28
    header = f"  {'Algorithm':<{col_w}}" + "".join(f"  {lbl:>18}" for lbl in active_labels)
    print(header)
    ...
    scalar_cols = [c for c in ("avg_waiting", "avg_slowdown", "avg_turnaround") if c in summary.columns]
    if scalar_cols and summary[scalar_cols].notna().any(axis=None):
        print()
        print("  Scalar metrics summary:")
        scalar_labels = {"avg_waiting": "Avg Wait (s)", "avg_slowdown": "Avg Slowdown", "avg_turnaround": "Avg Turnaround (s)"}
        header2 = f"  {'Algorithm':<{col_w}}" + "".join(f"  {scalar_labels[c]:>22}" for c in scalar_cols)
        print(header2)
        ...
        for lbl in labels:
            ...
            for c in scalar_cols:
                ...
                row_str += f"  {float(val):>22.2f}" if pd.notna(val) else f"  {'N/A':>22}"
            print(row_str)
...
def multi_line_plot(csv_paths: list, plots_dir: Path, show: bool) -> None:
    ...
    active = [col for col in METRIC_COLS if any(df[col].max() > 0 for _,_,df in all_data)]
    ...
    for allocator_name, group in groups.items():
        ...
        fig.tight_layout()
        save_figure(fig, plots_dir/partition/"multi", f"multi_line_{partition}_{allocator_name}")
        if show:
            plt.show()
        plt.close(fig)
...
def heatmap_plot(csv_paths: list[Path], plots_dir: Path, show: bool) -> None:   
    ...
    for partition, records in groups.items():
        ...
        matrices = {metric: df.pivot(index="allocator", columns="selector", values=metric).reindex(index=ALLOCATORS, columns=SELECTORS) for metric in METRICS}
        
        for i, (metric, matrix) in enumerate(matrices.items()):
            ax = axes[i]
            norm = (matrix - matrix.min().min()) / (matrix.max().max() - matrix.min().min())
...
def radar_plot(csv_paths: list[Path], plots_dir: Path, show: bool) -> None:
    RADAR_METRICS = ["max_waiting", "avg_waiting", "max_slowdown", "avg_slowdown", "avg_turnaround"]
    RADAR_LABELS = {
    ...
    metric_arr = np.array([[m[metric] for metric in RADAR_METRICS] for _,_,m in all_data])
    ...
    for selector_name, records in groups.items():
        ...
        fig.tight_layout()
        save_figure(fig, plots_dir/partition/"radar", f"radar_{partition}_{selector_name}")
        if show:
            plt.show()
        plt.close(fig)
...
def resolve_csv_paths(args: argparse.Namespace) -> tuple[list[Path], str]:
    ...
    if not result_dir.exists():
        print(f"ERROR: result directory '{result_dir}' does not exist.", file=sys.stderr)
        sys.exit(1)
