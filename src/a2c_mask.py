...
- __init__(...): mirror PPO-style API; keep use_masking path.
- _setup_model(): policy init, optimizer, maskable rollout buffer.
- _setup_learn(...): reuse lifecycle pattern from ppo_mask.py.
- collect_rollouts(...):
  - fetch action_masks
  - sample actions with masked distribution
  - store masks in rollout buffer
- train():
  - single-pass A2C update (policy loss + value loss - entropy bonus)
  - call evaluate_actions(..., action_masks=...)
  - log train/policy_loss, train/value_loss, train/entropy_loss, train/loss
- predict(..., action_masks=None): deterministic/stochastic masked inference.
- Guardrails:
  - validate mask shape/dtype
  - all-false mask -> fail fast with clear error.
'''
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get(batch_size=None):
            ...
            if rollout_data.action_masks is not None:
                values, log_prob, entropy = self.policy.evaluate_actions(rollout_data.observations, actions, action_masks=rollout_data.action_masks)
            else:
